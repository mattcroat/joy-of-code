...
- **You have localization** where you support multiple languages, but only load the language the user is using.unicode-range
...
Keep in mind if you're swapping characters using ****, the **@font-face** order matters.

You can learn more about some of these techniques by reading about [](https://developer.mozilla.org/en-US/docs/Web/CSS/%40font-face/unicode-range) on MDN, or listen to a great [@font-face explainer episode from The CSS podcast](https://thecsspodcast.libsyn.com/040-the-css-podcast-font-face).
...
I'm from 🇭🇷 [Croatia](https://en.wikipedia.org/wiki/Croatia), so we have other special characters such as "š, đ, č, ć and ž". These are included in [Latin Extended-A](https://en.wikipedia.org/wiki/Latin_Extended-A).
...
- The same user visits https://lesscoolsite.xyz/ which requests the same font
- This hits the HTTP cache and uses the cached version
...
Fonts come in many formats, the most popular being:
...
WOFF2 should always be used as it has a further **30% reduction in size** over WOFF.
...
- **font-style** — can be **normal**, **italic** or **oblique** (keep in mind you have to include an italic version of the font, since it only makes the regular glyphs slanted, and hideous)
- **font-weight** — is the numeric weight of the font: **100** (Thin), **200** (Extra Light), **300** (Light), **400** (Normal), **500** (Medium), **600** (Semi Bold), **700** (Bold), **800** (Extra Bold), **900** (Black)
...
On bad internet, the more things you have to download the more frustrating the experience. Let's adress other issues this causes.

## Flash of Unstyled Content

> A *Flash Of Unstyled Content or Text* is an instance where a web page appears briefly with the browser's default styles prior to loading an external CSS stylesheet, due to the web browser engine rendering the page before all information is retrieved. The page corrects itself as soon as the style rules are loaded and applied; however, the shift may be distracting. Related problems include flash of invisible text (FOIT) and flash of faux text (FOFT). - [Flash of unstyled content (Wikipedia)](https://en.wikipedia.org/wiki/Flash_of_unstyled_content)
...
We briefly went over **font-display** earlier, but this time let's explore it further. The values range from **auto**, **block**, **swap**, **fallback**, and **optional**. By default it uses **auto** which leaves it up to the browser, and most browsers use **block**.
...
**Fallback gives the font face an extremely small block period (100 ms), and a short swap period (3 seconds)** — similar to swap, if the font is nor ready it loads the fallback immediately. However, if the font doesn't load in a reasonable time, the fallback is going to be used.
...
**Optional gives the font face an extremely small block period (100 ms), and a zero second swap period** — you should use this with a font that's unimportant to how your site looks. Otherwise it's going to be a jarring experience.
...
You can play around with more examples on [Variable Fonts](https://v-fonts.com/) to understand what variable fonts can do. If you want to learn more, this [introduction to variable fonts on the web](https://web.dev/variable-fonts/) has great explanations with graphics.
