...
In the past we could only use a handful of fonts considered web-safe because they were guaranteed to be on each computer. Thanks to CSS, we can specify any font files to be downloaded, and used when someone opens our site. You probably used [Google Fonts](https://fonts.google.com/) — one of the most popular font providers. You only have to grab a link to a font, and include it in your site.
...
We use the **unicode-range** property to say: "**Hey, only load this font, if a character from this range is used**". This is useful in cases where:
...
For example, take the letter **A**. In Unicode this letter is represented as **U+0041**. These code points are in hexadecimal — preceded by **U+**. If we used a [Hex to ASCII Text Converter](https://www.rapidtables.com/convert/number/hex-to-ascii.html) converter, we would get the same result.
...
**Phew!** That's a lot of explaining out of the way. Understanding this isn't just for sake of curiosity, because this is the foundation of everything we're going to cover next. I'm going to cover more on **@font-face** in the self-hosting section.
...
The problem with using non-variable fonts is that you have to be careful with what you include. Including each font weight requires a separate file meaning an additional HTTP request, and declaring **@font-face** for each one (you can get around this by smashing fonts together into a single file).

For example if we wanted to use every weight of Inter, that's **9 HTTP request** in total. It gets worse — that's around **~1.8 MB** (assuming we're using **WOFF 2**, otherwise it's more than double). To put things into perspective, the regular weight of Inter is around **~98 kB**.
...
- **Layout features** — [feature tags](https://docs.microsoft.com/en-us/typography/opentype/spec/featuretags) such as **kern** let's us justify the space between glyphs, **liga** replaces a sequence of glyphs with a single glyph, or we can specify everything with the asterisk
