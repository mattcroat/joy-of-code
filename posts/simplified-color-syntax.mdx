...
You might believe CSS moves slowly, because the versioning stopped after CSS 3 — but it's very much alive. Instead of shipping large releases that takes years for features to hit browsers, we get them when they're ready.
...
For example `#3cb371` can be read as `3c b3 71` — so the shorthand would be `#3c3`.
