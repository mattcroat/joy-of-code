---
title: Use The Simplified CSS Color Syntax
description: CSS Color Module Level 4
published: '2021-4-11'
category: 'CSS'
image: '/images/simplified-color-syntax/og-image.webp'
---

# Use The Simplified CSS Color Syntax

You might think CSS moves slowly, because the versioning stopped after CSS 3 — but it's very much alive. Instead of shipping large releases that takes years for features to hit browsers, we get them when they're ready.
...
For example `#3cb371`, shorthand would be `#3c3`.
...
You can also use decimal `0.5` values instead of percentages `50%` for the opacity, but I prefer to be explicit.
