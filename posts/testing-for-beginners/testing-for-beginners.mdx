...
We're going to learn how to test any frontend JavaScript frameworks to have more confidence in our code.
...
Because Testing Library doesn't care about what you use to render the component and just gives you methods to do assertions.
...
Let's build a simple test framework in a couple of lines of code to see how it works. Credit goes to Kent C. Dodds and his great article on [But really, what is a JavaScript test?](https://kentcdodds.com/blog/but-really-what-is-a-javascript-test).
...
- **End-to-end** testing means simulating the actual user experience inside a browser for example navigating between pages for a Discord login integration ([Cypress](https://www.cypress.io/) is popular for this and works great with Testing Library but it's overkill for us)
...
The second thing most people ask is **"What to test?"**. This depends of course but you always want to test something that's most critical to your application. You might not have time or resources to test every aspect and that's fine so test the parts that are a disaster if they fail such as a checkout process.
...
[Jest](https://jestjs.io/) is a JavaScript testing framework and a test runner that gives you methods to test your logic.

[Testing Library](https://testing-library.com/) was made by [@kentcdodds](https://twitter.com/kentcdodds) and is only responsible for querying and interacting with DOM nodes thanks to [jsdom](https://github.com/jsdom/jsdom) simulating the browser in [Node.js](https://nodejs.org/en/) but it doesn't care how you render the DOM but it requires a test runner like Jest and works with everything like using a real browser with Cypress.
...
You can learn more about other methods for [appearance and disappearance](https://testing-library.com/docs/guide-disappearance/) that we're going to use later to wait for animations to complete.
...
<blockquote>
  🐿️ Note that we're using a case insensitive regex pattern to match the text
  because from the perspective of the user we don't care what case the text is
  using but that it's on the page.
</blockquote>
...
<blockquote>
🐿️ Here is everything you're going to need for the project if you want to
...
<blockquote>
  🐿️ `i` is short for `install` and `-D` is short for `--dev` which means
  development dependencies because these packages are only used in development.
</blockquote>
...
<blockquote>
  🐿️ Because the project uses ES modules with `"type": "module"` in
  `package.json` we have to use the `.cjs` extension for the older CommonJS
  module syntax.
</blockquote>
...
- `babel-jest` if required because we're using the ES modules `import` syntax which Babel has to transpile
...
```ts:example.test.js {showLineNumbers}
test('it should work', () => {})
```

<blockquote>
  🐿️ You're going to see tests placed in a `__tests__` folder as convention but
  it's not a rule because Jest is going to look for tests that include a `.test`
  or `.spec` extension.
</blockquote>
...
<blockquote>
  🐿️ If you need help join a Discord server of the framework you're using
  because everyone speaks the same language. You can ask how to test what you
  want. Provide a reproduction using [CodeSandbox](https://codesandbox.io/).
</blockquote>
...
<blockquote>
  🐿️ Jest exposes methods like `test` and `afterEach` globally so don't be
  alarmed.
</blockquote>
...
```ts:Todos.test.ts {showLineNumbers}
...
<blockquote>
  🐿️ When testing it's desirable to first make the test fail so you know it
  works when it passes to avoid false positives.
</blockquote>
...
Alright, you're on fire! 🔥 How would we test adding multiple todo items? Whenever something is new to you like a library or framework it's easy to forget it's just JavaScript. We already know how to use loops and encapsulate repeating logic inside functions, right?
...
In this case this happens when removing a todo so we have to use the `waitFor` and `queryBy` otherwise we would get an error from `getBy` because it's not in the document.
...
Because we're using `queryBy` it requires a container that we can get from the `render` as `container`. This is documented in the [API section](https://testing-library.com/docs/svelte-testing-library/api/) of the Testing Library documentation.

Another passing test! 👏

<blockquote>
  🧠 If you're up for a challenge at the end you can refactor repeating code if
  you wish like rendering the todo and querying the todo input so it's reusable
  inside a `renderTodo` function. You have to return `render` so you can use the
  methods.
</blockquote>
...
Now we can focus on testing individual components because we couldn't test the entire logic inside `Todo.svelte` since it requires more components working together so we did it in the parent.
...
Take an example the newsletter you can subscribe to below. You wouldn't use the real API because it might be slow or if it's not working it might break your tests so look into using [Mock Service Worker](https://mswjs.io/) instead.

As for other resources I highly recommend reading everything from @kentcdodds on [testing](https://kentcdodds.com/blog?q=testing) including [Common mistakes with React Testing Library](https://kentcdodds.com/blog/common-mistakes-with-react-testing-library).

If you want a structured approach to learning testing he made [Testing JavaScript](https://testingjavascript.com/) but it might not be for everyone how expensive it is for an individual. If you're working a job, they should pay for your education.
