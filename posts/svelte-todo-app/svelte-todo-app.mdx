...
# Make a Svelte Todo App Using Problem Solving

## Table of Contents

## The Project

This is an interactive example! 🦄 You can find the [source files on GitHub](https://github.com/JoysOfCode/svelte-todo).
...
I know from the title of the post some of you might be yawning 🥱 but stick around!

This is just an excuse for us to learn more about how using Svelte might look in a real project with animations and in another post I'm going to show you why and how to add tests in Svelte so you have more confidence your code works as expected and how testing can be fun.

We're going to be making a todo list app based on [TodoMVC](https://todomvc.com/) which is one of my favorite examples when learning anything new because it teaches us everything we need to know about what we're trying to learn because everything you build is creating, reading, updating and deleting.

## Learn From Reverse Engineering

I understand a lot of you might struggle with how to get started and I want you to know that's **normal**. The more you work on things the more experience you have and your tool belt helps you solve more problems like Batman (but please don't go around beating people up at night 🦇).

If we look at the [TodoMVC](https://todomvc.com/examples/vanilla-es6/) example we can observe a lot of things and that might be intimidating at first if you were asked to do it on your own. Don't concern yourself with everything at once. Break everything into smaller parts. It's mostly a text input field adding what you just typed to the screen.

Focus on one step at a time. 👍️

One thing I would be distracted by for example is how to get the animations working when you complete a todo and notice if you double-click edit it does a cool thing where you can edit the todo.

This might sound complicated, right? 😱
...
<Image
  width={1053}
  height={704}
  src="/images/svelte-todo-app/todomvc-editing.webp"
  alt="TodoMVC editing a todo item"
/>

Huh? it's not bad as you thought! If you see it's just hiding the label with the original content and replacing it with an empty text input if you're in the editing state.
...
Next thing I would do to learn how it works is to copy the HTML and CSS they're using. You don't even need JavaScript because you can just use fake data in the markup. In fact you can even disable the styles on the page by removing the `<head>` so you can focus on how it works.
...
<Image
  width={1053}
  height={704}
  src="/images/svelte-todo-app/todomvc-without-styles.webp"
  alt="TodoMVC todo list markup without styles"
/>

I observed how their example works and wrote down a todo list (I know, very meta 🤭) on what I need to make it work the same.
...
<blockquote>
  🖌️ Bob is here to help you out and let you know when you have to do something.
</blockquote>

<blockquote>
  🐿️ Peapod is a friend of Bob's and provides you with helpful information.
</blockquote>
...
This is the brief rundown of the folder structure:
...
🖌️ Remove these files because we don't need them:
...
Before we continue let's set up a **path alias** so we can import our files using `$root` which means the root of our project so if you have nested imports you don't have to do `../../some-file.svelte` gymnastics.
...
You can include the styles inside `index.html` but you wouldn't benefit from HMR (hot module reload) meaning you won't see updates instantly as you make changes so let's include the styles inside `App.svelte`.
...
I took the liberty to modernize the styles and make it more accessible from the original. There's also some slight differences in the markup but it's not relevant to us.

If you notice we already checked some of the items from the list such as the input being focused when the page loads and using a `<form>` we can use <kbd>Enter</kbd> to submit.
...
🖌️ Let's use some fake data inside `Todos.svelte` to have something to work with and loop over the list of to-dos.
...
<Image
  width={946}
  height={672}
  src="/images/svelte-todo-app/todos-list.webp"
  alt="Showing list of todo items"
/>
...
We're going to need a couple of props:
...
- **amount** so we know the todos count to show and hide marking the todos as completed

Parts of our application depend on the same values such as `todos` and `amount` but in the future you can use a Svelte store if you want.

🖌️ Let's keep it simple and define what we need inside `Todos.svelte` and simply pass the props to `AddTodo.svelte`.

```html:src/components/Todos.svelte {4,14-39,46} {showLineNumbers}
<script lang="ts">
  import type { ITodo } from '$root/types/todo'
...
```html:src/components/AddTodo.svelte {1-34} {showLineNumbers}
<script lang="ts">
...
If you want to you can add `$: console.log(todos)` inside `Todos.svelte` to see the values update when you change them so it's easier to see what's going on.
...
```html:src/components/Todos.svelte {5,42-49,60} {showLineNumbers}
<script lang="ts">
  import type { ITodo } from '$root/types/todo'
...
We're also going to set the `checked` attribute and use the `class:` directive to apply a `.completed` class if the todo is completed.

<blockquote>
  🐿️ If you're wondering about why we use the anonymous function `() =>
  completeTodo(todo.id)` syntax and not just invoke `completeTodo(todo.id)` it's
  because it would invoke the function immediately and run only once.
</blockquote>

🖌️ Update `Todo.svelte` to reflect the changes.

```html:src/components/Todo.svelte {14-15,26} {showLineNumbers}
<script lang="ts">
  import type { ITodo } from '$root/types/todo'
...
```html:src/components/Todos.svelte {51-53,64} {showLineNumbers}
<script lang="ts">
  import type { ITodo } from '$root/types/todo'
...
That's it! 🥳 Since we've done the groundwork for `Todo.svelte` it's going to be easier adding features.
...
```html:src/components/Todos.svelte {56-59,70} {showLineNumbers}
<script lang="ts">
  import type { ITodo } from '$root/types/todo'
...
We're going to have an `editing` state and `toggleEdit` function to change it together with a couple of methods for handling updating the todo:
...
After we're going to add a `class:editing` directive to the `<li>`, a `dblclick` event listener on the span to toggle editing mode and conditionally show the editing input element based on if `editing` is true with `keydown` and `blur` event listeners.

🖌️ Update `Todo.svelte`.

```html:src/components/Todo.svelte {13-42,45,58,71-80} {showLineNumbers}
<script lang="ts">
  import type { ITodo } from '$root/types/todo'
...
```html:src/components/Todos.svelte {18,76} {showLineNumbers}
<script lang="ts">
  import type { ITodo } from '$root/types/todo'
...
```html:src/components/Todos.svelte {2,7,17,22,66-79,89,96} {showLineNumbers}
<script lang="ts">
...
```html:src/components/Todos.svelte {8,24,83-85,103} {showLineNumbers}
<script lang="ts">
  import type { FiltersType, ITodo } from '$root/types/todo'
...
Stick around if you want to learn how to add permanent storage using local storage and animations.
...
First we deserialize the value from local storage using `JSON.parse` because we're going to store an array and values in local storage have to be a string. If there is nothing with the key of **todos** we default to an empty array using `??` which is the [nullish coalescing operator](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Nullish_coalescing_operator).
...
This is great but it would be even better if we used a Svelte store to be able to reuse it across components. This might look similar if you ever used React hooks.
...
We're going to see how generics are awesome in a moment.
...
```html:src/components/Todos.svelte {3,7,12-15,28,35,42,51,55,75} {showLineNumbers}
<script lang="ts">
  import type { FiltersType, ITodo } from '$root/types/todo'
...
Going back to TypeScript generics because we passed the `ITodo` type as a generic `<ITodo[]>` type for `useStorage` we know the type of `todos` is a writable that returns an array of todos or `Writable<ITodo[]>`. Sweet! 😄

I'm going to leave you with an exercise dear reader. 🧠 Since we're using a
store we can define and export the todos there so we can **collocate** more
things inside of the components because we no longer have
to keep state in the parent. I want you to clean up the code if you're up for
it.
...
That's it! 🦄 There's one problem though. The transition is happening for each todo item when we change the filter so let's fix that.
...
This one was a head scratcher but remember how I said to learn and use tools when the problem arises? This is a great example of learning about the Svelte lifecycle function [tick](https://svelte.dev/docs#run-time-svelte-tick).

The solution we can use is to have a computed `duration` value for the animation that's going to be `0 ms` or `250 ms` based on if `filtering` is `true` or `false` in `Todos.svelte` which we can pass to `Todo.svelte`.

There's one problem we're going to encounter.
...
Svelte batches pending DOM changes for efficiency so nothing would happen. You can see that's true if you log the computed value `$: console.log(duration)`.

Using `tick` we can let Svelte know to update the DOM immediately.

🖌️ Let's update `Todos.svelte` and pass the `duration` prop to `Todo.svelte`.

```html:src/components/Todos.svelte {2,16,67-73,99-105} {showLineNumbers}
<script lang="ts">
...
With that we can cross off all the items from the list we wanted to do at the start. In the next post I'm going to show you how to add tests. 🧪
...
Take your time learning and share it with others.
