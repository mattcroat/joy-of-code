...
description: Gain confidence through understanding how to use TypeScript
published: '2021-7-7'
category: 'TypeScript'
...
If you're learning TypeScript for the first time, or need a reminder this is the resource for you.

The hardest part of TypeScript is interpreting what it wants from you and picking the right approach. Even if you understand how some features of TypeScript work it's not always obvious when you would use it.

There's a lot of great resources out there for learning TypeScript but I always felt they use abstract examples that are hard to relate to and don't explain how to help yourself figuring out types.

That being said I urge you to try everything out and just dive in your own projects to learn. Nothing can substitute that experience. Use this as reference.

_note: this one was a massive one to write so there might be more typos than usual — I plan to go over everything and improve it even more over time, so thank you for any contribution you might have!_
...
28. [Type Assertion For Rebels](#type-assertion-for-rebels)
...
Have you ever wondered how autocomplete in your editor works?
...
Not only does it infer `pokemon` to be an **array**, but an **array of strings**.
...
Since it can only be an array the [editor intellisense](https://code.visualstudio.com/docs/editor/intellisense) gives us array methods.

Imagine for a moment not having this feature. You would have to remember and look every method up on the [MDN Web Docs](https://developer.mozilla.org/en-US/).
...
We can also see the signature call of the method. It's like having the documentation in your editor. This is purely JavaScript enhanced by our editor!

You're already using TypeScript. This is the theme you're going to notice throughout. It's not just types — it's the entire developer experience.

We can add `@ts-check` to the top of our file to enable TypeScript checking:
...
Since it infered the array to only contain strings it throws a type error because we're adding a number to it.

We're going to learn later how this works. First let's answer our initial question.

How does this work?

**The VS Code editor uses TypeScript under the hood.** The same **TypeScript language server** is open source, so it's ported to other editors.
...
**The language server provides us with sophisticated features such as code completion, refactoring, syntax higlighting, and error and warnings.**
...
The majority of web developers use [Visual Studio Code](https://code.visualstudio.com/) from [Microsoft](https://www.microsoft.com/en-us/). They also made [TypeScript](https://www.typescriptlang.org/), so it works out of the box.

If you're using [WebStorm](https://www.jetbrains.com/webstorm/) it also has great TypeScript support. You might have to get a plugin.

If you're a [Vim](https://www.vim.org/) user you can also find TypeScript plugins. You're already using Vim, so you can figure it out.

You can get the same support as you would in VS Code for any editor.

What your editor can do for you goes beyond:
...
VS Code is so incredible you can write a book about it while being intuitive and simple to use.

## What is TypeScript?

**In JavaScript we don't get a lot of information before we run our code.**

It's often the case we just hope it works. We run the code and squint at our console to read the error before we go back to our editor and fix it.
...
We made a slight typo and got `undefined`. Our editor didn't warn us about this. Imagine this being an API and passing wrong arguments to a method.
...
This would be an error in TypeScript.
...
Our editor is able to point out and fix the problem for using the quick fix feature.
...
**Static type checking means your code is evaluated before it runs to ensure it's more predictable and works as expected.**

You should also keep that limitation in mind.
...
- **Runtime** is when our JavaScript code gets executed
- **Compile time** is when TypeScript code gets transpiled to JavaScript code

**TypeScript only checks your code at compile time.** This means you can't ignore checks in your code to prevent the user from doing unexpected things.

We can't include TypeScript annotations in `.js` files, so we have to use the `.ts` extension.
...
addPokemon('Pikachu') // ['Pikachu'] 👍
addPokemon(1) // Type 'number' not assignable to type 'string'. 🚫
```

We're just saying that the argument `pokemonName` should be of type `string`.

Note that **we can run TypeScript code despite type errors** because those are not syntax errors. This also depends how strict your compiler settings are.

Done! Right? Ship it. Actually...
...
addPokemon('Pikachu') // ['Pikachu']
addPokemon(1) // ['Pikachu', 1]
```

TypeScript didn't betray us. We forgot to put checks in place for our user.
...
function addPokemon(pokemonName: string) {
  if (!pokemonName || typeof pokemonName !== 'string') {
    // handle error
    errors.push('💩 Oops! You have to specify a Pokemon name.')
    return
  }
...
As you've seen in previous examples we can reap the benefits of TypeScript without using TypeScript directly.

You can also leverage [JSDoc](https://jsdoc.app/) and TypeScript to add type annotations without using TypeScript and have self-documenting code. If that's something you're interested in. You can read more on [JSDoc support](https://jsdoc.app/) and look at the examples. I'm just going to focus on TypeScript.

Type checking can be strict as you would like. This prevents us from being overwhelmed and makes adding types a gradual adoption.

I'm going to explain more on configuring the TypeScript compiler later.
...
- TypeScript is a **superset of JavaScript** (JavaScript program is also a valid TypeScript program)
- TypeScript **preserves the runtime behavior of JavaScript** (TypeScript doesn't change how JavaScript works, it only adds rules how different kinds of values can be used)
- TypesScript **compiles to JavaScript**
- **Types get erased once it compiles to JavaScript** (things like browsers don't understand TypeScript)
- TypeScript is a **gradual adoption**

## JavaScript Types

_"You don't need TypeScript, we have TypeScript at home"._

Have you ever heard a parent say something similar? 🤭
...
JavaScript also has **primitive wrapper objects**:
...
Let's clear up the difference between primitives and their object counterparts so you don't get confused between if you should use the lowercase `string`, or capitalized `String` as a type.

```js:example.js
// string primitive
const stringPrimitive = 'Pikachu' // 'Pikachu'

// string object
const stringObject = new String('Pikachu') // String { 'Pikachu' }
```
...
**JavaScript converts primitives to their object counterparts behind the scenes so we can use their methods.** This is because methods like `.toUpperCase()` extend the `String` object.

The reason we don't use the object syntax is because it's more work to get the value and could give us unexpected results if we pass an object to something expecting a primitive value.

Don't confuse the `new String(value)` constructor with the function `String(value)` that performs type conversion to a primitive string.

You see a frequent use of this when you need to convert a string to a number:
...
We can use JavaScript primitive types as TypeScript types. Doesn't that mean we just end up writing more code? As we've learned TypeScript doesn't save us at runtime, so we have to put checks in place:
...
Why are we writing more code? Let's take `addPokemon` for example.
...
- Is `addPokemon` callable?
- Does it return anything?
- What are the arguments?
- What date format `added` accepts?

Even if we look at the implementation we don't understand what type of date it expectes us to pass. This is when we turn to reading the documentation.

**TypeScript helps us prevent making those mistakes in the first place.** It let's us know we're accessing the right properties and passing the right arguments with code completion.

With TypeScript we can define the **shape** and add **constrains**:
...
We assign the argument `name` to type `string` and argument `added` to type `Date` which is just a custom built-in type.
...
You can imagine how this can be useful when dealing with an API of a library. You don't have to constantly switch between some documentation and your code.

```ts:example.ts
addPokemon('Pikachu', Date()) // Type 'string' is not assignable to parameter of type 'Date'. 🚫
```
...
TypeScript saves us from ourselves. The `Date()` function returns a `string`. We want the `new Date()` constructor that returns an object. That's what TypeScript is complaining about when it says it can't assign `string` to `Date`. The shape doesn't match because it's of type `object`.

```ts:example.ts
addPokemon('Pikachu', new Date()) // [{ name: 'Pikachu', added: Date... }] 👍
```
...
This is just to give you a taste and understanding of why you would use TypeScript.

We're going to start learning about TypeScript and using it in practice.

## TypeScript Playground

To get started open the [TypeScript Playground](https://www.typescriptlang.org/play). It's a great way of seeing the transpiled JavaScript code and generated TypeScript types without having to set up anything.

It's using the same [Monaco editor](https://microsoft.github.io/monaco-editor/) that powers VS Code, so you should feel at home.

On the right side there's a couple of notable tabs:

- **.JS** is the compiled JavaScript output (the default target is ES2017 which is adjustable from the **TS Config** tab)
- **.D.TS** are the generated types
- **Errors** are like the error logs in your console
- **Logs** show the output of your code

You can press <kbd>Ctrl</kbd> + <kbd>Enter</kbd> to run the code. Another tip I have is to include `console.clear()` at the top so your logs stay readable.
...
<Iframe
  ...
  src="https://www.typescriptlang.org/play?#code/MYewdgziA2CmB00QHMAUByAKgTwA6wGVgAnAS1wBcACUiKgQwHdYoBbWKwXg3AqnfQEogA"
/>

If you can't see the right sidebar press the arrow icon in the top right.
...
pokemon = 'Charizard'
pokemon.toUpperCase() // 'CHARIZARD' 👍

pokemon = 1 // Type 'number' is not assignable to type 'string'. 🚫
pokemon() // This expression is not callable. 🚫
```

<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/DYUwLgBADg9g1iAtjAdhAvBA5ABQJZwCGAxgBYCuWAUFbAsmplgMKmEBOeAXhwCbV0kqAHRgYAVShQQ7ZoQDOIABQBKGoIYYIARlrwhKVUA"
/>

If you hover over `pokemon` you can see TypeScript infered it as type `string`. Inspect the lines with errors. How great is this instant feedback in your editor?
...
It's encouraged to let TypeScript infer what it can. I personally prefer to be explicit on returns.

Let's take a fetch API example:
...
For now it's only important we focus on `any`. It's an escape hatch when we don't know what the type is. We can type the JSON object from the API response. This let's us be more explicit about the type:
...
Now when we use it we get code completion since TypeScript knows what the values are. There's a lot more the API returns. This is just an example.
...
Try the autocomplete on `pikachu.id` and `pikachu.name`. It's magic. 🪄

Don't sweat the details. We're going to go in-depth later and explore how to make this more organized by using other TypeScript features.

## Type Annotations

We can explicitly add a type annotation to specify the type:
...
apiResponse.doesntExist // we don't get any warning 😱
```

<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/MYewdgzgLgBAhgBwJYCUCmEHgmgXPMATxgF4YBvAKBhgBM4o58BtAXUoF9LLFUMtIaAHS0QGMFACiADyTQgA"
/>

It's useful when:

- You're working with some library that lacks types
- You encounter a complex type from an API you don't feel like tackling right now
- The API could change while you're writing the code

**Don't let your editor bully you.** If you're the type of person that wants to focus on writing code I suggest instead of using `any` everywhere to include `// @ts-nocheck` at the top of your file. After you're done writing code turn typechecking back on.
...
const anyType: any = apiResponse // assignable to `any` 👍
const unknownType: unknown = apiResponse // assignable to `unknown` 👍
const otherType: { data: [] } = apiResponse // 'unknown' not assignable to type '{ data: []; }'. 🚫

// we have to use checks to narrow down the type
if (apiResponse && typeof apiResponse === 'object') {
  apiResponse.data // Property 'data' does not exist on type 'object'. 🚫
}
```

We narrowed down what the type of `apiResponse` is. Yet we can't access the `data` property since it's `unknown`.

We can use type assertion to say to TypeScript we know better:
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/MYewdgzgLgBAhgBwJYCUCmEHgmgXDAVzAGswQB3MGAXhgG8AoGGAEzijnwG0BdBgXwYNQkWHDABPACoSEeeJJrxk6TNjTDssIqQpgZc-DrKUliVBiyQNI6DBBQAFmgBOB+XVbtOMXjH5mKpbqQkgAZjAAFOaqVjgwAGQJMFCyaCARMcHWNNS0AOQgAEYAVmjAUPkAlPRMzDC2sC7Z8bRZajlwEPReHNw8-nXMzR04AHRsHAJAA"
/>

We're going to learn about type assertion later. I just want to give you a feel of `unknown` since most examples don't.

We can use `unknown` to accept any argument without using `any`:
...
We can use `unknown` to describe a function that returns a value of unknown type:
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/MYewdgzgLgBAVhcMC8MAGAoG2YG8s6EBEAlgCZEBcMAjADQGHZFgCGAtgKZUxEAKJANatgACwCuRRjAC+GTBgBm4sMCgkkEVos59WAJwicAFADdWAG3Gdq0fSTABzAJTUVgsCADuYPNP2cUOL6vgBSAMoA8gByAHQADgZGZpbWzhhyGKCQsCAARnAoMFo6eoYmCODOQA"
/>

If we didn't use `unknown` the infered return type would be `any` meaning we could do whatever we want with `obj`. Try it out yourself.

If you're unsure when you'd use `any`, `unknown` it's fine. They're mostly useful when you're migrating existing JavaScript code to TypeScript. Don't look hard when to use them.

## void

`void` is a return type used when a function doesn't return any value. This doesn't mean it can't have any value like `undefined`.
...
pokemon = undefined
pokemon = 'Pikachu' // Type 'string' is not assignable to type 'void'. 🚫
```

<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/DYUwLgBADg9g1iAtjAdgLggNxgSwCYBQBsCyKEAvBAK4p4gBmOKIhJSqlEA5AAo5wAhgGMAFtW5A"
/>

You see `void` on functions that don't return anything:
...
Imagine a `forEach` implementation:
...
Let's just focus on `void` in the example. Because we used `void` as the return type for `forEach` we're saying the return value isn't going to be used, so it can be called with a callback that returns any value.

Using `void` explicity can save us from returning a value on accident during refactor:
...
function logPokemonRefactor(pokemon: string[]): void {
    for (const item of pokemon) {
        // ...
        return pokemon // Type 'string[]' is not assignable to type 'void'. 🚫
    }
}
```
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/GYVwdgxgLglg9mABAGzgcwApwNYFMC2CAFAA44EIBciAzlAE4xhoDaAugJTUBucMAJogDeAKETjEZPITAA6YHHoBRAIYQAFqXIzEAXgB8wsRJMB6U4llXjJ8fVxQQ9JFIpgbiAL4cRnkSNBIWAQUdCxpBAAlXGA1KEUtCLBqOkZmdi5EXgEjEwV6RCIIBDpEGCgCRDhgSW0EDlzbcXNLaya7BycXOvcTP08gA"
/>
...
- No reachable end point like a while loop or error exception
- Variables get the type `never` when narrowed by type guards to remove possibilities (one great example are React props where if you pass one type of prop you're saying other props can't used)
...
function getPokemonByType(pokemonType: 'fire' | 'water' | 'electric') {
  ...
  // type never can't have any value
  const remainingPokemonTypes: never = pokemonType // Type 'string' is not assignable to type 'never'. 🚫
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#"
/>

This let's us know that `getPokemonByType` is missing a check for the `electric` type.
...
I want to have it organized in a logical way so you can skim over certain parts and have a reference instead of sprinkling concepts throughout and being hard to find.

## Array Types

There's two ways to specify a type of array in TypeScript.

Using the generics `Array<Type>` syntax:
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/MYewdgzgLgBADiA1gUwLbgKoQJZgOYDiyYyATtsACoCecyAXDAIKmkCG1APNOfgHwwAvDADaAcgBCAVwA2AIzYQ2U0mIA0MMQGEAFm1Ko2YACZl1mgMoBHKdlJQZyMQF0AUKEiwEKdGCy48ayl9ZAl2YBQoCEYeAJFnIVFJWQUlFXNtPQMjU1UNMSC7BydnIA"
/>

You can use either one. I prefer the second one because it's less to type.

## Function Types

You can specify the inputs and output type of values:
...
We explicitly type `pokemon` as `string[]` to constrain what we can pass in. If TypeScript can't infer the type it uses `any` by default. In this case `any[]`.

You can let TypeScript infer the return value. You can also be explicit and require a return type. It's mostly preference.

The syntax for arrow functions is the same:
...
For anonymous functions TypeScript makes use of **contextual typing** to figure out the type of the argument based on how it's used:

```ts:playground.ts
const pokemonList = ['Bulbasaur', 'Charmander', 'Squirtle']

pokemonList.forEach(pokemon => console.log(pokemon)) // 'Bulbasaur', 'Charmander', 'Squirtle'
```

<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/MYewdgzgLgBADiA1gUwLbgDIEtowLwwDaA5AEICuANgEYCGEt5ATsQDQzEDCAFrU6rTAATZC3bEAygEdyWJlErJiAXQBQqhCnRhs0AHQAzEEwCitYNwAUmtOHwA+GKEghFeyiADm1pLbABKf1UgA"
/>

TypeScript infered `pokemonList` to be `string[]`. Because of this inside `forEach` it knows the individual `pokemon` type should be of type `string`.

Knowing it's a thing you understand what things you don't have to type.
...
There's no type error when we don't include the optional `hp` argument.
...
This is how you do "function overloading" in JavaScript:
...
In TypeScript we can specify a function that can be called in different ways by using overload signatures:
...
You might rarely write use this but you're going to see it used frequently when using a library someone else typed.
...
You can go through the overload signatures to understand what you can pass in.

## Object Types

To define an object type it's like writing a regular object in JavaScript.

You might notice if you hover over `pokemon` TypeScript already knows it's shape:
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/MYewdgzgLgBADiA1gUwLbgBIgG7IE4CyyMAvDAN4BQMMYAhqsgFwwDkACgJaJ3AAWAV1aUAvpUqhIsBCnRgW5Wg2YxoeTmADmMEaQrUljFh269BwkUA"
/>

We can specify optional properties:
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/PTAEAUCcHsAcFNIBcCeoDkBLAJu0mBnUAW0IMwDsBzfC0VBDAb1AoENj4AuUApSSlQDcoAL54ARgFckoSPACOUzPOy16KRuhY4eFKcQmIR7Tjz4DqI8QDpQgXg3A1XsAoAMbQKfULGgBreMQeALJkglBwiKg8Oth6BkaQADSsHNy8-IJioAC8oEzOoIUpZhjgmL5srgAWUujOos7OIKAABjgt+ERwSJgebAA2DoCyO24eXj7+gRQA8rA9ff0AgpBUBvAUSNH42AD8cYaIyaZpFpmiOXkFRcc86GUV1bX1jc3TEgBW8K6y-ZhIiAMSGw0B5+mgCAhXJgAGZoXwUaAAdzosBgCGQmHgBDsTlGnlkEwCHgAogAPfhscLoqJ5bZ7VjxQ7FE4ZahZXL5IrbHgARkSV0KN1K5UqNXQ-K5DDS6Hg-S+GVcdVEQA"
/>

This might be hard to read at first. We're going to learn ways to abstract these types to make it more readable.

## Union Types

TypeScript let's us combine types in different ways.
...
logPokemon(['Bulbasaur', 'Charmander', 'Squirtle']) // ['Bulbasaur', 'Charmander', 'Squirtle']
logPokemon('Pikachu') // Pikachu
```

<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/GYVwdgxgLglg9mABAGzgcwApwNYFMC2CAFAA44EIBciAzlAE4xhoDaAuogD60NNoCUiAN4AoRIggIacZLgB0qNKXKEw-EQF8RIxVjyqiLAOQAhEMgBGAQxpWQ9IwBpERgMIALK-XxWwAE1wHZyMAZQBHEBh6KFkjNnVdFWIjDBhsKwh3ECN+IA"
/>

We're saying that `pokemon` can either be an array of Pokemon `string[]`, or a single Pokemon of type `string`.

You can only do things with the union type that every member supports:
...
We can just narrow the type as before:
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/GYVwdgxgLglg9mABAGzgcwApwNYFMC2CAFAA44EIBciAzlAE4xhoDaAuogD60NNoCUiAN4AoRIhjBERAIL16AQwCeAOhg05ipaXKEw-QaPHiICGnGS4VqNDrx6V+BSTsUkAXgB8iMvYQqoOABVEhJcegBhBRpcIgN+MUQAXxFEyWkoJTC4KV83RHdCxAByOkZmYsNEkzMLKxtXB0CQsMjo2INElJSRGyw-MCIWYoAhEGQAI2iFEHpigBoSiIALBXonMAATcIWSgGUARxAYeihLYrYEvt1iYowYbAUIZZBKoA"
/>

The second `if...else` statement is not required since if `pokemon` isn't of type `string[]` it can only be `string`. Keep in mind TypeScript doesn't save us at runtime.

In cases where the union members like `string[]` and `string` have same methods like `slice` you don't have to narrow the type:
...
logPokemon(['Bulbasaur', 'Charmander', 'Squirtle']) // ['Bulbasaur']
logPokemon('Pikachu') // 'P'
```

<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/GYVwdgxgLglg9mABAGzgcwApwNYFMC2CAFAA44EIBciAzlAE4xhoDaAuogD60NNoCUiAN4AoROMQB6SYgDucethqJgCxACM4UABaIoATxK5lgXg3AsjtiJEBDTjJcAOlRpS5QmAc1kMCLiIAGABpEAEZ+fhEAXxERZyw8dyIWAHIAIRBkdQBDGiyQemTg5IBhbSz6fCywABNcAqKAZQBHEBh6KHtktgi4t2JkjBhsLIhtEGT+IA"
/>

## Discriminated Unions

A **discriminated union** is when you **narrow** the members of the union that have the same **literal type**.
...
// 🔥 'Flamethrower'
pokemonAttack({ pokemonType: 'fire', flamethrower: () => console.log('🔥 Flamethrower') })

// 🌀 'Whirlpool'
pokemonAttack({ pokemonType: 'water', whirlpool: () => console.log('🌀 Whirlpool') })
```

<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#-MoBA4Jh-o1mq13J4OmCGn1BsNRuNJlEZgQFstVutNsBtipdvtDscQKdzpdrndHs8Gq9yutFCo1H9qfpAcDQfo-H5iPDboBSXdcADFqLQGExWF5iK84gkEElCJFMqYLFZXHYgdYADQUEVfcXQLh8QQiBD4XJqCDlSjoADm3Gs-OQws+Yp+Xn4Pl4-nhgB4NwAA+64AOryBmqSjeaXxRLcBWvGqqtweaBatYh5Rhw1aE1mi1W232-3IYMKdNqawe3hAA"
/>

The type-checker can't determine if `flamethrower` or `whirpool` is present based on `pokemonType` property. We know more than TypeScript.

We have to be more intentional about what we know to TypeScript:
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/JYOwLgpgTgZghgYwgAgGLCig3gKGf5GAGzgFsIwALKAewHdoAuZACgEpkBeAPmQDcawACZ4CABxoBrCKRogAKgE8xEZgHIYGCGpwBfHDlCRYiFAHU4x5LgLI6lDEQk0izdl14Dho-BOmyFZVVkNTpLaB19HDAg5AAFKRk5LjQtZAAfZAtjAxgAVxAEMGBkvySQAEEwMERJFjKA5gT-OQ4bAgBnOmAwBEpWBrkAOkHAlTafW2QEOA6UDS01Rkmp8USAoeIyCmp6aHYV1eQAI0w4SUP8GbmQsOMly6nRoftHZyIDo6nTiHPJ-SiAHpAchALwbgFJdkKoEjkKi0BhQHSjKo1BB1LDIUZKFTqTSYNQAGkIMJ28KYrA4PGmcg6LggQyINAA5iw1BC0CS4XtERxdGwDMDkIAeDcAAPshMwOKBOGguJHrOQo2osDFYoLqO4RImvKXvNyU3gIGl0hnM1mirKS6Wy3lsIA"
/>
...
This also helps us catch mistakes if something passed through the `switch` clauses that shouldn't.

## Type Aliases

So far we've been using types directly in type annotations. This is hard to read and not reusable.

A type alias is as the name suggests — just an alias for a type. Think of it as a shorthand.

Just how there's people with different _types_ 🤭 of personalities sharing the same name.

Take an array of Pokemon:

```ts:playground.ts
type Pokemon = { id: number, name: string, type: string }
...
pokemon.forEach(pokemon => console.log(pokemon)) // { 'id': 1, 'name': 'Pikachu', 'type': 'electric' }
```

<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/C4TwDgpgBACg9gawgWzgOygXigbygSwBMAuKNAV2QCMIAnAGjIENkJSBnYW-NAc0dCQOXHrygBfAFCSAxuk5QwiFOlLwkqNAG0AulihackqARJQAjPWPNWpAOQx8CJjIAW5O1ZOC2UOxAAbCBkRGTtJcR1pJQ10ADoAMzhaAFEXVwAKGJUMTAA+KDk0djgguIC4XizlTQBKWqA"
/>

Types can get long and unwieldy. We can alias and reuse them across our code. We can use a `types.ts` file where we import these types from.

Note how we use `Pokemon[]` to say it's an array of type `Pokemon`. We're not just limited to primitives. We can also it this way:
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/C4TwDgpgBACg9gawgWzgOygXigbygSwBMAuKNAV2QCMIAnAGjIENkJSBnYW-NAc0dCQOXHrygBfANoBdAFCyAxuk5QwiFOlLwkqDNkk5ZUAiSgBGekeatSAchj4ETBQAtyty8cFsotiABsIBREFW1lxOVk1HXQAOgAzOFoAUWcXAApojT0APiglNHY4QNj-OF5M9V0ASmqgA"
/>

I prefer the first example because of semantics. If we have a single `Pokemon` type we can specify elsewhere it's a list of Pokemon with `Pokemon[]`. Else we would have to make another type like `Pokemons` (even if Pokemon is already plural) to indicate there's many.

We used an object type but it can be anything:

```ts:playground.ts
type Pokemon = string[] | string

function logPokemon(pokemon: string[] | string): Pokemon {
  if (Array.isArray(pokemon)) {
...
console.log(logPokemon(['Bulbasaur', 'Charmander', 'Squirtle'])) // ['BULBASAUR', 'CHARMANDER', 'SQUIRTLE']
console.log(logPokemon('Pikachu')) // 'PIKACHU'
console.log(logPokemon()) // Please enter a Pokemon.
```

<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#"
/>

Notice how we're using the same `Pokemon` type as the argument and the return value for `logPokemon`.

Functions are just special objects in JavaScript:
...
This is reflected in the types because we can just describe the shape of an `object` including the call signature.

Using a a **type alias** to type a **call signature of an anonymous function**:

```ts:playground.ts
type LogPokemon = (pokemon: string) => void

const logPokemon: LogPokemon = (pokemon) => console.log(pokemon)
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/C4TwDgpgBAMg9gcwApwNYQLZwHZQLxQAUYamOAXFAM7ABOAltggJT4B8UAbnPQCYBQ-AMY4aUADaIU6LNkrxkpWfiIkZOVng4jsVOOIgA6SQmJKNgk9LLZCAciT1UAQyEALAK53mQA"
/>

This doesn't work for regular function declarations (named function expressions).

Using a **type alias** for **construct signatures** used to define the type signature of a constructor function:
...
console.log(pokemon) // PokemonFactory: { "name": "Pikachu", "pokemonType": "electric" }
```

<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#"
/>

Most of what you see here is not exclusive to **type aliases**. We can swap the `type` syntax for `interface` and it would work the same.

We're going to learn more about it next.
...
We can mark `object` properties optional with `?` and type function call signatures `attack(): void` just like with **type aliases**

Interfaces and type aliases are almost interchangable. You can also use a semicolon or period after each property. It's optional.
...
The short answer is — doesn't matter. Pick one and if it doesn't work for you use the other one. You can even use both. It's more about semantics.

- Interface is more appropriate for describing shapes of objects
- Interfaces and type aliases can be extended
- You can add new fields to an existing interface but not to a type alias

Extending a type alias using intersections:
...
type Pikachu = Pokemon & { type: 'electric' }
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/C4TwDgpgBACg9gawgWzgOygXigbwFBSFQCWAJgFxRoCuyARhAE4FFoCGyElAzsI8WgDmeAL548oSLGII2AYwAW1LLEQp0UAGS4okrlADkEADYQ5fYnINQxeAPR2oCtt11wo3NsGLcAZiF0FaE9OKEUzBFdAXg3AWR28OXReKDAZeSVKGFTFZWx8IhIKKABGABoWQnZOSgNM2WyDMvy9apMzCytRIA"
/>
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/JYOwLgpgTgZghgYwgAgAoHsDWEC26TIDeAUMmcsACYBcyIArjgEbSnkhw4S0DOYUoAObEAvsWKhIsRClTBMiABb1kEAB6QQlHmiy58RNmTABPAA7dkAcggAbCAn7AEV0eID075Irg6w6ZB44MGAeGBNkMEUUIK5kBGiETB1AXg3AWR3iBHw+ZDN5JXpaOQUElQBeQ3IKGmQARgAaIzpOSytigqtGqtMLWht7RwEXUSA"
/>

Interfaces can get you into hot water if you don't remember that you can use an existing interface on accident:
...
// Type '{ color: string; style: "double-hung"; }' is missing the following properties from
// type 'Window': applicationCache, clientInformation, closed, customElements, and 207 more. 🚫
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#"
/>

`Window` is already an existing type that's declared in the `lib.dom.d.ts` DOM types file for the global `window` object.

There's some controversy around how you should name your interfaces. People from other languages might be used to using the `I` prefix. In TypeScript it's discouraged. I leave that to your judgement.

## Intersection Types

**Intersection types** let us combine types.

To combine types we use the `&` operator:
...
This is what I meant when I mentioned that you can use **type aliases** and **interfaces** together. Use them in situations where it makes sense.

I threw in a sneaky **tuple** in `pokemonType` because a Pokemon can have dual-types. We're going to learn more about it later.

We could have **extended** the **interface** instead:

```ts:example.ts
interface FirePokemon extends Pokemon, Ability, Moves {}
```

The difference is negligible. I think **type aliases** were more appropriate to use here.

## Type Assertion

Type assertion is useful when you know more about a type than TypeScript. You can use it to specify a more specific type or override a type. Think of it like type casting.

In this example TypeScript only knows that `document.getElementById` returns some kind of `HTMLElement`:
...
If we tried to use the form `reset` method:

```ts:playground.ts
const formEl = document.getElementById('form')

formEl?.reset() // Property 'reset' does not exist on type 'HTMLElement'. 🚫
```
...
We can be more specific about the type of element by using type assertion with the `as` keyword:
...
The 🧠 pro-tip of how you figure what element you want is fumbling around until your code completion gives you an option that looks like you want.
...
Don't confuse this syntax with TypeScript generics. We're going to learn about generics later.

This isn't popular because React components are named like that. We get this error in the TypeScript Playground because in the TS Config JSX for React is on by default.
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?jsx=0#"
/>

In the example I disabled JSX in the TS Config options.

In JavaScript and even it's frameworks event listeners are something we use often.

Imagine we have a input field to get Pokemon name:

```html:example.html
<input type="text" id="pokemon" />
```

Our first lesson is how we can rely on TypeScript to infer types, so we can figure out the type to use:
...
If you hover over `event` in `addEventListener` you're going to notice it infered the type `Event`. This makes sense because [Event](https://developer.mozilla.org/en-US/docs/web/api/event) is the base interface for events.

Knowing that we can type out `event` in `handleInput`:
...
Using type assertion we can solve both of these problems:
...
- We're saying to TypeScript that `targetEl` can't be `null` with `event.target!` (we should do checks to be safe)
- At this point `targetEl` has the type `EventTarget` (this corresponds to the web API)
- Since we want to access `value` which is a property of `HTMLInputElement` we have to help out TypeScript here by using type assertion
- Now we can use methods and properties of HTMLInputElement

Derived from Event there's [UIEvent](https://developer.mozilla.org/en-US/docs/Web/API/UIEvent) that has other interfaces like [MouseEvent](https://developer.mozilla.org/en-US/docs/Web/API/MouseEvent), [TouchEvent](https://developer.mozilla.org/en-US/docs/Web/API/TouchEvent) and [KeyboardEvent](https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent) among others.

You don't have to keep this knowledge in your brain. Just let TypeScript and your editor help you:
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/PTAEFMDdwOwF1ASwM6gAYFkD2BXZ4BRaeNAKAHdEYATLcgOgENrqjY4AZFOWcAJwAUAcgC2ufFmh8hAGlACo7AJSgAvAD5QAYywxkWADbh6BrAHNh2POAjE4QpUtLOQt9klRoAKri0ALNhIKKloGZlY7LmQeGH5hOF8-MWhZeUV4FQ1tXX0jE3NhHxx-N3gHJxcwdIQUdABpcABPACMsRj4I9jJKGjomFkDObl5BIQBrJpwAB1SFO0zNHT1DY1MLIQaWto7S+0cgA"
/>

Also keep in mind that when using type assertion you're saying the element can't be `null`:

```ts:playground.ts
const pokemonInputEl = document.getElementById('pokemon') // HTMLElement | null

const pokemonInputEl = document.getElementById('pokemon') as HTMLInputElement // HTMLInputElement
```

Using type assertion can cause problems since you won't get warned about code that can go wrong. Make sure you have checks in place.
...
Use this only when you're sure the value exists. Don't lie to the TypeScript compiler.

## Type Assertion For Rebels

Using `any` and type assertions is great when you're just writing code but avoid using them unless you have to.
...
That being said you can do whatever you want (you rebel):

```ts:playground.ts
const pokemon = 'Pikachu' as unknown as number

pokemon.toFixed() // no complaints, but wait until you run your code 💥
```
...
This works because TypeScript only allows type assertions that convert to a more specific or less specific version of a type.

That being said you might run into some scenarios where this is helpful, so keep it in mind.
...
If you hover over `pokemon` the type is `string`:
...
When you hover over `pokemon` the type is `Pikachu`:
...
If we combine literals into unions we see how it's useful:

```ts:playground.ts
function movePokemonOnBoard(direction: 'up' | 'right' | 'down' | 'left') {
  // ...
}

movePokemonOnBoard('up') // acceptable value ✅
movePokemonOnBoard('rigth') // oops! typo. 🚫
```

<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/GYVwdgxgLglg9mABAWzgNwKYAU4GsOpgDyYAQnAIYBOAJgBQ0xUbTxgBciA5CAA5eIAPtyowA5gAsoA4VxpwA7mBncANhmDSAlIgDeAKERHEAehOIAdFf0BffftSYc+QiXLV6PfjrOIKECAxeKAoAI3VENApVEAxEQFByB3RsPAIEN0paOi5RMSgJLh9zODheAGcAQkQoAE9eOAtEQF4NwGq9oA"
/>

There's also `number`, and `boolean` literals. `boolean` itself is just an alias for the `true | false` union.
...
addPokemon(pokemon.name, pokemon.type) // pokemon.type is 'electric' ✅
```
...
pokemon[bulbasaur] // careful if this is what you want ⚠
```
...
pokemon[bulbasaur] // correct ✅
```

<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/MYewdgziA2CmB0w4EMBOAKAlAKGwSzABdZUAzZYWAAgGVDlCIqBvbK9qvAEwC4qwArgFsARiTYcAFgAc+g0eI5UG9YAGs5wsagnsusUrEixNCnQF9cBYmQrUACiDWwh4FrqoiB0EcgjIBVD46BggPYEk0IWQwfSDaekYPCABHATxUQjhgxLDLbFBIQippJxdwPkdnVzAqAF53JS8fPwD45k5eKgBGABoqGT4AFgBWfpUKDSohgE5+-UNjYZmqc17wyNRo2JI+Du4+ACZ+waoAZjnlQlUpkeOqBaMIE2mz1fWlVPTM7JZOvjOJ1k0yG42uk2GAA55gYni8AGwjVbYfKFCDFZq+fyBPjo1AEADm9SoAHJMa1AiSCuB0VQIlEYnE+CT6VtGSQScSWZttnEqXBil8MllqA0SUKfrBOX46TTCLhStVwABtcnY1AAXWwivKYGVrN5JC1OpqyolIq1QA"
/>
...
Because TypeScript knows _everything_ 👀 it can save us from some JavaScript quirks:
...
This mistake is part of the language. You can [The history of “typeof null”](https://2ality.com/2013/10/typeof-null.html) if you're interested.
...
// Argument of type 'number' is not assignable to parameter of type 'Length'. 🚫
...
// Argument of type '"oops"' is not assignable to parameter of type '"hp" | "name" | "pokemonType"'. 🚫
...
TypeScript compiles Enums down to some _interesting_ 🧐 JavaScript code you can look at in the JavaScript output tab if you're curious.
...
🗺️ Cartesian coordinates anyone?
...
**Interfaces** are _implemented_ 🤭 using the `implements` keyword:
...
If you're a Node chad you can use [ts-node](https://www.npmjs.com/package/ts-node) that let's you run TypeScript files with `ts-node app.ts`. You also get a live code environment (REPL) if you run `ts-node` (`npx ts-node` 👍) like you do using `node`. If you're using `ts-node` you can use [nodemon](https://www.npmjs.com/package/nodemon) to watch the files by creating a `nodemon app.ts` script in `package.json`.
...
<Image
  height={768}
  width={1024}
  src="/images/typescript-fundamentals/24-array-es5-interface.png"
  alt="es5 interface"
/>
...
<Image
  height={768}
  width={1024}
  src="/images/typescript-fundamentals/25-array-constructor.png"
...
const chosenPokemon = pokemon[bulbasaur] // TypeScript is mad. 🚫
```
...
> Element implicitly has an 'any' type because expression of type 'string' can't be used to index type ... **because** no index signature with a parameter of type 'string' was found on type ...
...
// it expects the literal type `bulbasaur` ✅
...
There's a lot more to learn about TypeScript. Using TypeScript to type out a library is another game. Did you know you can use conditional logic in your types? I'll leave that for another post. 🤭
