...
If you're on the fence about TypeScript but like using [JSDoc](https://jsdoc.app/) you can take advantage of TypeScript being built-in. You can use JSDoc for types and have self-documenting code that can outlive TypeScript. You can read more about [JSDoc support](https://code.visualstudio.com/Docs/languages/javascript#_jsdoc-support) for VS Code and look at the examples.
...
```ts:example.ts
// Type 'string' is not assignable to parameter of type 'Date'. 🚫
...
If you hover over `getPokemon` you can see TypeScript infered the return type as `Promise<any>`. `any` is an escape hatch when we don't know what the type is.
...
Thanks to this slight change we get code completion for the `pikachu` object since TypeScript knows the type. It's magic. 🪄
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#"
/>
...
`void` is a return type used when a function doesn't return any value but it doesn't mean it can't have any value like `undefined`.
...
There's two ways to specify **array type** in TypeScript and they're equivalent.

Using the generics `Array<Type>` syntax or using the `type[]` syntax.
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/MYewdgzgLgBADiA1gUwLbgKoQJZgOYDiyYyATtsACoCecyAXDAIKmkCG1APNOfgHwwAvDADaAcgBCAVwA2AIzYQ2U0mIA0MMQGEAFm1Ko2YACZl1mgMoBHKdlJQZyMQF0AUK9CRYCFOjBZcPGspfWQJdmAUKAhGHkCRZyFRSVkFJRVzbT0DI1NVDTFguwcnZyA"
/>
