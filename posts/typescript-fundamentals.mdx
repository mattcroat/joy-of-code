...
If you do use **VS Code** you're probably like me and don't even know anymore what's an extension and what's built-in.
...
TypeScript isn't just about types — it's popular because this is the kind of developer experience it gives.

We can add `@ts-check` to the top of a JavaScript file to enable TypeScript checking in JavaScript.
...
If we look at the built-in **VS Code** extensions, we can notice some usual suspects. Look, it's [Emmet](https://www.emmet.io/)!
...
The editor code completion already makes it harder to make such a mistake writing regular JavaScript because of the benefits we get from TypeScript under the hood, but when we do there's nothing to warn us.
...
function addPokemon(pokemonName: string) {
  if (!pokemonName || typeof pokemonName !== 'string') {
    // handle error
    errors.push('💩 You have to specify a Pokemon name.')
    return
  }
...
Even if we look at the implementation of `addPokemon` we don't know what date format we should pass to `added`, so we turn to documentation.

TypeScript helps us prevent make those mistakes in the first place by knowing we are accessing the right properties and passing the right arguments alongside code completion.
...
In the example the argument `name` is of type `string` and argument `added` is of type `Date` which is just a built-in type.
...
This is extremely useful when dealing with some API because the documentation lives in your editor.
...
TypeScript can also **infer** the return type of a function. If we're not returning anything it's `void` (more on that later).
...
If you want to focus on writing code I suggest instead of using `any` everywhere to include `// @ts-nocheck` at the top of your file. After you're done writing code turn type checking back on.
...
- You must perform checks to **type narrow** before you can use a value
...
- Variables get the type `never` when **narrowed by type guards** to remove possibilities (one great example is preventing impossible states when a prop is passed to a component where you can say if one type of prop gets passed another can't)
...
We're going to learn ways to abstract types to make them more readable and reusable.
...
**You can only do things with the union type that every member supports** meaning that you can't just use a `string` method without checks because you said to TypeScript the type is `string[] | string`.
...
This saves us from creating another type and using weird grammar like `Pokemons` to indicate there's many since Pokemon is already plural.
...
The 🧠 pro-tip of how to figure out what element you want is fumbling around until your code completion gives you an option that looks like the option you want.
...
// no complaints, but wait until you run your code 💥
pokemon.toFixed()
```
...
```ts:playground.ts
function addPokemon(
  name: string, pokemonType: '🔥 fire' | '🌀 water' | '⚡ electric'
) {
...
// `pokemon.pokemonType` is 'string' instead of '⚡ electric' 🚫
...
// pokemon.type is 'electric' ✅
addPokemon(pokemon.name, pokemon.pokemonType)
```

<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#"
  load={false}
/>

TypeScript adds a `readonly` type on the `pokemon` properties that signals they won't change, so TypeScript infers a **literal type** instead of a **general type** like `string` or `number`.
...
This shows the [Pokemon API response](https://pokeapi.co/api/v2/pokemon/charizard).

We're using an **index signature** syntax `[key: type]: type` to indicate that the keys of the object are going to be a `string` with a `unknown` property.

This is useful where we don't have control over an API or time to type out a complex interface.

In the previous example we might only care about some properties and not the rest.
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/JYOwLgpgTgZghgYwgAgAoHsDWEC26QCCqAkgEoQDOADvhSgN4BQyLyA9G8gDYRjJwgAnsnRgAFtGRUo6KtDDDxMgK4BzMcnFwwzVh2Q5tCCRU0TkoACYQAHsgrBVIbcqgRdLANpXbALntgUKCqALr+yiCYIOgA7iCMHuycAEZwdMi2ckEQIEiJqXQA+pnQwDlI-iDKOMnQCaxJUli4+BaWicCWldW1UPV6nDTYeCDIzjjuDeMQ-hSBwf0s+kMto3DJwFzAYGUUieub27v+BFBQcIIAPPT8G1sK-jfTs--vBxL6ADyyUTEFpFlM6BgmkEcmQ-YiUViIH7duQgF4NwDVe3WZegeHay6oWUPgK2oJY7eg79A7WB0ABlV6Xnljhr6GIoEAIAgdtjFnTBTFAKQuEQFAYEfKQZCyHZKGQf9kEsfB+z4TcYludBlB0RVaFPCBz3LFkrzEJsbzbB8nygfhTCqGpoB5F930-asfwGVCUB7ZBVDcbQG3QawRJwKgeBLKl4OkWR5BQtCcLwgiQyo4c7ztA57l2O04KgABRRAxDZBoWRubTtmEAEaw1OVQxTM8LxZSyFDtaY01YYM8wvPsQGrRp+wAdn7T5+2SaDmwgUK10cpsAFoaH-KB+yAA"
  load={false}
/>

Here's another example where we have Pokemon ratings but don't know the Pokemon ahead of time.
...
In reality we want to access the `pokemon` object using the **type literal** type `bulbasaur` instead of `string`.
...
What you should do is make sure you're passing a **type literal** instead.
...
You could also use the `as const` assertion.

## Type Narrowing

**Type narrowing** is when you narrow types to more specific types, so you can limit what you can with a certain value.
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/GYVwdgxgLglg9mABAcwKZQApwNaoLYIBCAngCrEAOqAFAFCIOIU74LlUBciA5MDAE6puiAD48A7gEMoqfsLHdUAG1TR+MCN1oBKRAG96iAPRHEUSqkQwAzjz6D5E6bMeKVajVoYxgias1wCMHZLAF5wuwEhXQNGRkEoEH4kbkBeDcBSXcR7S3MqRCxAhC9EAF9aQxNEcUswSX5+OHErKEQAE0akKDgnGTlRHmVVKHVNQx8-ANZgi0Rw0J6XGMM4hKSUwB4NwCh9qud+MxmCqeKyitNqxFr6xua2jrNutyGR4smgkPKGSpCAZQh1Cha2DAjVsCCUxAG7mGnistmY1msMAARipDKtkjxAIVkiEGHgg+zyhyCWhOaEwLCCJBC1EeeO42iAA"
  load={false}
/>
...
The next example shows how we can use the `in` operator that checks if a property is in the object for **type narrowing**.
...
This mistake is part of the language. You can read [The history of “typeof null”](https://2ality.com/2013/10/typeof-null.html) to learn why that is so.

## Type Predicates

**Type predicates** are a special return type that's like a **type guard** for functions.

Take for example a `isPokemon` function that checks if a passed value is a Pokemon. In `filterPokemon` the `items` array is of type `unknown[]` so `item` is `unknown`. We expect TypeScript to narrow the type through the `if...else` clause to make sure the item we're passing is a Pokemon, so we can access it's properties but the `item` is `unknown`.
...
We can use **type predicates** to narrow the `item` type to `Pokemon` by using the `argumentName is Type` syntax that changes the argument type if the function returns `true`.
...
To make the code a lot more readable we can use a **type alias** or **interface**.
...
Let's look at an example where we use `logPokemon` to log Pokemon where we don't know the type ahead of time because a user could pass anything from a `string`, `object` or `array`.
...
Hovering over the values we can see the infered type for `logPokemon` is `(pokemon: any): any` and `log1` and `log2` are `any`.

We might think of using a **union type** to handle the types but it's not ideal since it wouldn't narrow the type.
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/GYVwdgxgLglg9mABAWwIYAcAUqBOOBciqYAngDSISoA21ARqhANaHY4DmhxJAlIgLwA+IqT4BvAFCJpiHAFMoIHElw4AdGixVaDZjwkBfCRIgIAzlETA4ONFAAKcJnOQIBKDJikyA2gHIAIRB6VDNUJT8KPwBhAAtcNDAAEzkcSMQ-AGUARxAYHChqOT8AXTJvaUx0JxcEPiFEaudXMDUoOABVdHRU6NC5TH19CQB6EcQfACIAjoAZAIBBTIWOgCVJiknogAkF1YBZBYA5ABEAUXXNzIBFDoBJVYAVWbPJkpNzOCK1ajh2TGstlQDhqLR4QA"
  load={false}
/>
...
Using proper names for **generics** makes the code easier to read.

```ts:playground.ts
function map<Input, Output>(
  arr: Input[], callback: (arg: Input) => Output
) {
  return arr.map(callback)
}
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/GYVwdgxgLglg9mABAWwIYAcA8BJM6RQA0iA8gflAHwAUAUIg4qgE7MBciuFA2gLrERUAGyEAjVBADWHaiwDmHLgQCUiALyVS5ArVUBveo2YBTKCGZIWzAHRp01QSPFTltAL61aEBAGcoiYDhmNCgABThJY2QEdRQMOkZEbgByACEQMVQfVHNk4mSAYQALFjQwABNjZjzEZIBlAEcQGGYoIWNk-kMGanQIqIRVDUQ+yOiwayg4AFV0dCqCrONqV1daAHp1pIAiVOmAGVSAQTqj6YAlbeJtgoAJI-OAWSOAOQARAFFL67qARWnsOcACr7D7bXheXxwdrWIRwOTUQLBVBhfrjZRAA"
  load={false}
/>
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/JYOwLgpgTgZghgYwgAgAoHsDWEC26TIDeAUMmciHDhAFzIDOYUoA5qeQBYAOdIArjgBG0YgF9ixMAE8uKAMpg4YZAF5kAcm7qJMPiARhg+BuihgM2PCAA8AFRkoIAD0ggAJvTRZc+AHwAKdjIubys6e1kAbQBdABoghkUwOgUlYgBKcIcYogSoCDA+KAIQy3wAOnpTMH9-GGAoRgBRABtYhggEfDdW9NVfDq73VsjGJWjkAFpkesawEbGwaPSxCSHGZFKfEDoLbZy1SISSclOKKloNAGEOOCgcOHdodXizzh5kAGYATgTRV-IJze52odHUNzu1BaEHwLwSp24dAArAAOP4AshAt6UUHXW7MABedzccOByERyAA7GjTv9iNEJAB6RkmMxeMoEQRScnAFgcCAbRbEdboaHlFroFj+KpmPZWfxbKztTRcdTpdJAA"
  load={false}
/>
...
In situations where we want to know if properties of an object we're trying to access exist we can use the `keyof` operator.
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/GYVwdgxgLglg9mABAcwKZQAoCc4AdVZQCeAPACpH4A0iA0qkYqgB5SpgAmAzogNYNxgiCvgB8ACgBQiGYjgAjAFYAuYZVRVps-kVX0ikgJSIA3lplZ0ILEgWKA2joC6kgL6TJEBFyiJccfgBbBEQAXlNzRAALXFUAZgBWTVlEMABDQNRVAHIMGF40iCiQbOTZfyCEESzEbNQAG1RoLBgIbLcPNEwcfEIicQrUYLAabJjsw0ku7DwCYgGAoYRRuDwuCaA"
  load={false}
/>
...
There's a lot more to **enums** if you want to [learn more about enums](https://www.typescriptlang.org/docs/handbook/enums.html) since I rarely use them I just wanted to give a brief overview.
...
The `public` member can be accessed anywhere and it's the default. You don't have to type it unless you want to be explicit.
...
TypeScript adds the concept of an **abstract class** that's a contract used by other classes to extend from. The **abstract class** doesn't contain implementation and can't be instantiated.
...
In the next section we're going to briefly introduce another concept that TypeScript introduces to classes — **decorators**.

## Decorators

**Decorators** are like **higher-order functions** (function that takes another function as an argument or returns a function) that can hook into a class and it's methods and properties, so we end up with composable and reusable pieces of code.
...
I want to preface this by saying that **decorators** are primarily used by library authors and frameworks such as [Angular](https://angular.io/) and [Nest](https://nestjs.com/).

Don't sweat it when you would use **decorators** because they mainly just improve your developer experience without having to think about how it works.
...
In the next example we're going to see how we can use a **method decorator** for validation.

To check if a Pokemon has enough experience to evolve we add **@requiredExperience** above the `evolve` method.

The implementation is inside `requiredExperience` that's also known as a **decorator factory** because it returns a function that will be called by the **decorator** at runtime.
...
Hooking into code like this is also called [metaprogramming](https://en.wikipedia.org/wiki/Metaprogramming) and is often more useful in frameworks where you might need to add metadata to measure or analyze code.

There's a great video by [Fireship](https://www.youtube.com/channel/UCsBjURrPoezykLs9EqgamOA) on [The Magic of TypeScript Decorators](https://www.youtube.com/watch?v=O6A-u_FoEX8) you should watch. 🍿
...
You're probably learning TypeScript to use with a JavaScript framework and that's great because most popular JavaScript frameworks require zero configuration for enabling TypeScript support. You should consult the docs for your framework how to set TypeScript up.

If you want to use vanilla TypeScript on a passion project I highly recommend you use [Vite](https://vitejs.dev/) to set up your project. Using Vite is simple as running `npm init @vitejs/app` and picking a template.

I would use Vite for any project and you can look at the list of [Vite template presets](https://vitejs.dev/guide/#scaffolding-your-first-vite-project) for other frameworks.

Let's look at how to set up TypeScript (it's easy) from scratch and show you around the TypeScript compiler settings.

Start by initializing a project inside an empty folder and installing TypeScript as a development dependency.
...
Next create a `app.ts` file at the root and add some TypeScript code.
...
The `tsc` command invokes the TypeScript compiler that creates a `app.js` file. This is what your browser and Node are going to run.
...
It's easier to add a command to scripts in your `package.json`, so we can just use `npm run dev`.
...
If you're not using Vite and want live reload for your site you can use [live-server](https://www.npmjs.com/package/live-server).

You can run live-server and TypeScript at the same time.
...
If you're a Node chad you can use [ts-node](https://www.npmjs.com/package/ts-node) that let's you run TypeScript files with `ts-node app.ts`.

You also get a live code environment (REPL) if you run `ts-node` (`npx ts-node` 👍) like you would typing `node` in your terminal.
...
Let's make the `tsconfig.json` file more readable by removing most of the TypeScript compiler options and focusing on a single options.
...
If you want to fire up a quick TypeScript demo you can use an online editor like [CodeSandbox](https://codesandbox.io/) so you can try things out.
...
This is more useful than code completion since you can dive into the types directly to understand the surface area of an API.

If you just started to learn TypeScript it requires practice to understand the types first.
...
There's always going to be a lot of information in type definition files. Focus on what you're inspecting. We can leap-frog from type definition to type definition by doing the same to other types inside.
...
This has the properties we would expect like `length`, `pop`, `push` and `map`.
...
Let's not get overwhelmed and read the error calmly.
...
The further you go down the chain the more specific the error is, so start from there.
...
We just want TypeScript to infer the type for us here so we don't have to do it by hand.
...
The problem is that we're not passing the **type literal** `bulbasaur`, so TypeScript can compare it to the key `bulbasaur` in `pokemon`.
...
You have to keep in mind these older libraries use **commonjs** that uses module exports, so we can't expect it to use **ECMAScript modules** where you can import a package simply as `import http from 'http` using the default export or `import { http } from http` if it's a named import.
