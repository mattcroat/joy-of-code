...
So why are we writing **more** code? Let's look at an example of a `addPokemon` function but don't understand the implementation details.
...
For whatever reasong using the prefix `IPokemon` for an **interface** is controversial inside TypeScript circles but you probably shouldn't care as long as your naming convention is consistent.
...
**You can only do things with the union type that every member supports** meaning that you can't just use a `string` method without checks because you said to TypeScript the type could either be a `string[]` or `string`.
...
Unfortunately the second `if...else` statement is required because TypeScript can't **narrow** down the `pokemon` type to `string` itself.
