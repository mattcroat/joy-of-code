...
If you use **VS Code** you're probably like me and don't even know what's built-in and what's from an extension anymore.
...
TypeScript isn't just about types — it's popular because this is the kind of developer experience you can expect.

We can add `@ts-check` at the top of a JavaScript file to enable TypeScript checking in JavaScript.
...
So why are we writing **more** code? Let's look at an example where we're just adding Pokemon and don't understand the implementation details.
...
TypeScript helps us **prevent** from making those mistakes in the first place by knowing we are accessing the **right properties** and passing the **right arguments** alongside **code completion**.
...
```js:example.js
function logPokemon() {
  console.log(logPokemon.pokemon)
}
...
What you should do is make sure you're passing a **type literal** instead or use the `as const` assertion.
...
You only have to include the JavaScript file in your HTML file. Using `defer` loads it after the DOM (Document Object Model) has loaded.
...
This problem might be harder to reason about if you don't understand things like **object index signatures** in TypeScript and that's why everything we learned so far ties together.
