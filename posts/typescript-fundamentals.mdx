...
Sometimes that's not enough when dealing with a framework like React where component names are capitalized so you can use a suffix such as `PokemonType` for a **type alias** or a prefix `IPokemon` for the **interface** that's for some reason controversial around TypeScript circles.

You probably shouldn't care and you can use any name you prefer such as `PokemonProps` as long as your naming convention is consistent.
...
The 🧠 pro-tip of how to figure out what element you want is fumbling around until your code completion gives you an option that looks like what you want.
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#code/JYOwLgpgTgZghgYwgAgAoHsDWEC26QCCqAkgEoQDOADvhSgN4BQyLyA9G8gDYRjJwgAnsnRgAFtGRUo6KtDDDxMgK4BzMcnFwwzVh2Q5tCCRU0TkoACYQAHsgrBVIbcqgRdLANpXbALntgUKCqALr+yiCYIOgA7iCMHuycAEZwdMi2ckEQIEiJqXQA+pnQwDlI-iDKOMnQCaxJUli4+BaWicCWldW1UPV6nDTYeCDIzjjuDeMQ-hSBwf0s+kMto3DJwFzAYGUUieub27v+BFBQcIIAPPT8G1sK-jfTs--xM3xVWA21rpknEvoAPLJPMQJUWUzoGCaQRyBkRKKxED07nIQC8G4Bqvb77vQPG5zdUmrT3PQUGsUG5YHQAGVXrfDdOGvoYigIAQBAljIMYS6YKYoBSFwiAoDAj5SDIWQ7JQyAAcglj4PSfC7jEtzoMoOiurQl4QNeLaanezQjA+T6SGkYw9NAhqvh+X6aj+YL-igY7IKobjaGB6DWMJOBUDwjbyoh0iyPIaEYXhBFEYk94HPcuzcghUAAKKIGI2oNA0mo3Op2zCACPY0k6KZkRRt5mQo3JTokSbVjeE4gJq9Jcpg+nKPSPb6PSADMACs9KfPSczaMACCRQyWzqGAIDBAAtDIlj0kAA"
  load={false}
/>
...
Take for example a `isPokemon` function that checks if a passed value is a Pokemon. In `filterPokemon` the `items` array is of type `unknown[]` so `item` is `unknown`. We expect TypeScript to narrow the type through the `if...else` statement to make sure the item we're passing is a Pokemon, so we can access it's properties but the `item` is `unknown`.
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#"
  load={false}
/>
...
<Iframe
  title="TypeScript Playground"
  src="https://www.typescriptlang.org/play?#-jgnVsIjgCmowVCqAig0mqiAA"
  load={false}
/>
...
There's more to **enums** if you're curious you can [read more about enums](https://www.typescriptlang.org/docs/handbook/enums.html) since I just gave a brief overview.
...
If you want to use vanilla TypeScript on a passion project I highly recommend you use [Vite](https://vitejs.dev/) to set up your project. Using Vite is simple as running `npm init vite` and picking a template.

I would use **Vite** for any project since it's the future of web development (you won't want to go back once you try out the developer experience).

You can look at the list of [Vite template presets](https://vitejs.dev/guide/#scaffolding-your-first-vite-project) for other frameworks.
...
If you're not using **Vite** and want **live reload** for your site you can use [live-server](https://www.npmjs.com/package/live-server) and run **live-server** and TypeScript at the same time.
...
You only have to include the JavaScript file in your HTML file using `defer` to load it after the DOM (Document Object Model) has loaded.
...
If you're a **Node** chad you can use [ts-node](https://www.npmjs.com/package/ts-node) that let's you run TypeScript files without having to transpile them first by using `ts-node app.ts`.
...
If we go back and look at **lib.es5.d.ts** on the right it looks like something we would expect. You can double-click on it or <kbd>Alt</kbd> + <kbd>Click</kbd> on the type definition to open it.
...
This problem might be harder to reason about if you don't understand things like **object index signatures** in TypeScript. Everything we learned ties together.
...
If you have doubts you can always ask a question in the [TypeScript Community Discord Server](https://discord.com/invite/typescript).
...
[DefinitelyTyped](https://definitelytyped.org/) is a project that holds a repository for types that everyone gathered around to contribute types to that has over **76,000** commits and it's used to [search for types](https://www.typescriptlang.org/dt/search?search=) on the official TypeScript page.
