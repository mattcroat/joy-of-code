...
You import another Svelte component using the `import Component as './Component'` syntax. Pretend that `artists` is some data we fetched as a JSON response from the Spotify API.
...
You might be asking when you'd use slots over regular components and the answer is you might not often reach for slots and that's fine.
