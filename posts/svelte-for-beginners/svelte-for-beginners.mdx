...
Ponder for a second how amazing it is that this is valid HTML in the first place and doesn't break things because browsers are so smart.
...
**Svelte is a superset of HTML meaning it's like writing actual HTML and JavaScript but with superpowers.**
...
When you're developing an application you want a change in your application state like adding a song to a playlist be reflected immediately in the DOM and re-render only what changed.
...
Sometimes you need to change a value based on other values. This is often referred to as a **computed property**. Svelte has **reactive declarations** using the `$:` syntax which is valid [JavaScript label syntax](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/label) that Svelte stole for itself. Using the `$:` syntax is saying **"re-run this code whenever any of the referenced values change"**.

```html:App.svelte {10, 31-34} {showLineNumbers}
<script>
	// state
	let album = [
...
This might be hard to wrap your head around at first but it makes more sense if you **think about it as giving Svelte dependencies to watch and rerun the code when the value changes** such as `album`.
...
```html:App.svelte {4, 13} {showLineNumbers}
<script>
 	let list = ['React', 'Vue', 'Svelte']
  let filteredList = []
	let searchQuery = ''
...
You might have an `<Artist />` component that gets passed the `artistName` property and an `<Album />` component passed `albumTitle` and `albumTracks` using `<AlbumTrack />` to list the songs. The `<AlbumTrack />` component has `track` and `length` properties but also a `playing` state.

You import another Svelte component using the `import Component as './Component'` syntax. Pretend that `artists` is some data we fetched as a JSON response from a Spotify API.
...
You might not often reach for slots or be confused when you'd use them over regular components and that's fine because there's not a good answer no matter how often you hear the word composition.
...
You aren't limited to transitions. In Svelte you can define custom animations such as this [typewriter effect](https://svelte.dev/tutorial/custom-js-transitions), use [spring and tweened motion](https://svelte.dev/tutorial/tweened) and make smooth transitions between elements using [flip animations](https://svelte.dev/tutorial/animate).
...
However, one feature you're going to use all the time is the [Svelte store](https://svelte.dev/docs#run-time-svelte-store) which is Svelte's answer to **global state management** but it's a lot more. You would reach for a store if you have **information that is required by multiple unrelated components** such as the logged in user or theme.
