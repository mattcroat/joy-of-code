...
published: '2021-9-7'
category: 'javascript'
image: '/images/template-strings-syntax-highlight/og-image.webp'
---

# Syntax Highlight and Code Completion Inside ES6 Template Strings

I love to dable in the fine arts of **vanilla JavaScript**, and one of my favorite things when doing so is using [template literals](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals) as **templating** because it's faster and more fun than using the **imperative** API of the browser to manipulate the Document Object Model (DOM).
...
We didn't even add **classes**, **attributes**, or nested elements that would make the code harder to read and more error prone compared to using **template literals**. I'm **not** saying it's bad — but it sucks the fun out of it.
...
That's how I found [Inline HTML](https://marketplace.visualstudio.com/items?itemName=pushqrdx.inline-html) — a Visual Studio Code plugin that solves those problems.
