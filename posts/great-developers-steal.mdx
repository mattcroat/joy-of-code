...
published: '2021-3-7'
category: 'JavaScript'
image: '/images/great-developers-steal/surreal-art.webp'
---

<Image
  src="/images/great-developers-steal/surreal-art.webp"
  alt="Surreal art morphing into each other"
  gif={true}
/>
...
- Why open source is faster than any other method for learning
...
There's a lot of packages on NPM. Take a package like [is-odd](https://www.npmjs.com/package/is-odd) with over half a million monthly downloads that does what the name implies — checks if a number is odd. It doesn't take a genius to realize we probably don't need a dependency for a single line of code that's `(n % 2) === 1`. It even includes another dependency [is-number](https://www.npmjs.com/package/is-number) with ~45 million weekly downloads! Let me make one thing clear. I'm not trying to shame, or make fun of anyone. I'm just trying to point out how many of us reach for these things. Your favorite libraries, or frameworks include some of these as dependencies.

What I'm trying to say is that before we include a library, we should consider if we need the entire library. In a recent project I had to convert a number to a fraction, and vice versa. I only had one problem — I'm math-illiterate. Turning a fraction to a decimal was simple enough, but doing the opposite not so. I was looking over at StackOverflow threads, and being reminded of my years in school while I was resting my eyes in math class. First I found [mathjs](https://mathjs.org/) and thought "sweet" to myself. It can easily do everything I want. I used [BundlePhobia](https://bundlephobia.com/result?p=mathjs@9.2.0) to check the size, and to my surprise it was almost 160kb — if that doesn't mean anything to you, that's larger than the framework (React) I'm using. 😱
...
I found [number2fraction](https://www.npmjs.com/package/number2fraction) searching NPM. I decided to look into how it works. I could have done the same with mathjs, but it was using [fraction.js](https://www.npmjs.com/package/fraction.js) as another dependency. The package is tiny in comparison, but the same would hold true. We can search for the name of the method used in the API to find where it's located. In this case it's `fraction2number`. If you can't find what you're looking for, clone the repo to your machine. You can also try importing it to [CodeSandbox](https://codesandbox.io/). This is why I'm excited about [GitHub Codespaces](https://github.com/features/codespaces).

Anyhow, the search lead me to a [couple of results](https://github.com/rimara14/number2fraction/search?q=fraction2number). I could quickly identify the one I'm looking for being [number-to-fraction.ts](https://github.com/rimara14/number2fraction/blob/master/src/number-to-fraction.ts). Files including `.spec` are also helpful. They're test files to ensure the code works as intended. It's useful to us, because we can see how it's used if we have to.

Don't be intimidated at looking at TypeScript files. TypeScript is JavaScript. You can ignore type annotations, if you don't understand them. You can always copy the piece of code, and remove TypeScript to not distract you. In a lot of situations, the code should also have some helpful comments.
...
// for this example we're going to assume 1.5
export function number2fraction(number) {
  ...
  if (!isDecimal.test(number.toString())) {
    // if not, return string representation
    // because the function expects return to be string
    return number.toString()
  }

  // store stringified number, for example '1.5'
  const stringifiedNumber = number.toString()
  // return amount of numbers after the decimal point
  // where the subtracted amount represents the number, and period
  // so it works for '1.5', but would break for '10.5'
  ...
  // if we read about https://en.wikipedia.org/wiki/Greatest_common_divisor
  // we can see it can be used to reduce a fraction
  const divisor = greatestCommonDivisor(numerator, denominator)

  // before we got the GCD, we had a fraction of 15/10 and
  // calculcated the divisor to be 5, now we only have to
  // return the fraction 15/5 equals 3, 10/5 equals 2
  // meaning 1.5 is equal to 3/2, if we want to simplify this further
  // to 1 1/2 we can learn how by using https://www.calculatorsoup.com/
  // which is a great way to learn how it works step by step
  return `${numerator / divisor}/${denominator / divisor}`
}
...
Sometimes a package you include works, but lacks a feature you'd want. You can contribute to the project, but "your deadline was yesterday". There could be a lot of other reasons why you'd want to do this. Maybe a package is so critical, you don't want any chance of it being compromised. Either way, in your project structure you can create a `vendor` folder to store your third-party code. The only downside is that you're also the maintainer.

## Conclusion

Looking at things through the lense of code can crystalize things for us. We don't need to have a deep understanding of everything. If we gain enough knowledge, and insight to gain confidence that things aren't as scary as we believe it's only a benefit to us.
