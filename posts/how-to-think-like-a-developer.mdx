...
If you don't know what project to work on, I'm going to share some resources here:
...
I like the idea behind the [TodoMVC](http://todomvc.com/) project. We're going to create a similar todo app. The styles are going to be simple. They're included in the source code, so we can focus on what's important.
...
```html:index.html
<input placeholder="Add a todo" data-input />
```
...
```js:app.js
const addTodoBtnEl = document.querySelector('[data-add]')
      ...
      ${todos
        .map(({ id, todo }) => {
          ...
          <li data-todo="${id}">
            ...
            <input data-input style="display: none" />
            <button data-update>Edit</button>
          </li>
        `
        })
        .join(' ')}
...
I'm adding inline styles temporarily. Later we're going to remove it in place of a stylsheet.
...
function updateUI() {
  ...
  todoListEl.onmouseover = ({ target }) => {
    ...
    const labelEl = parentEl.querySelector('[data-label]')
    const inputEl = parentEl.querySelector('[data-input]')
    const btnUpdateEl = parentEl.querySelector('[data-update]')
...
function updateUI() {
  ...
  todoListEl.onmouseover = ({ target }) => {
    // ...
    const btnDeleteEl = parentEl.querySelector('[data-delete]')
...
Lastly, this is how delete todo works:
