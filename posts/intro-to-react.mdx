...
description: Intro to React, and Frameworks in General
published: '2021-2-9'
category: 'React'
image: '/images/learn-react-easier/banner.webp'
---

<Image
  height="300px"
  width="1000px"
  src="/images/intro-to-react/banner.webp"
  alt="Person sitting on the React logo"
  banner
/>

# Intro to React, and Frameworks in General
...
**This isn't about prerequisite JavaScript knowledge you should know**. If you're looking for that, I recommend reading [JavaScript to Know for React](https://kentcdodds.com/blog/javascript-to-know-for-react).
...
After the ashes of [Angular.js](https://angularjs.org/) (not to be confused with [Angular](https://angular.io/)) transition to Angular 2 which was in reality an entirely different framework to the ire of many developers, people flocked to React (created at Facebook) in protest where it became and stayed one of the most popular frontend JavaScript frameworks.

Unlike Angular, Vue, or Svelte that are more opinionated (think of it as the swiss-army knife of frameworks) meaning they come with everything included out of the box such as state management, a router, perhaps a nice CLI (command-line interface) to manage everything. React took a different approach, and is completely unopinionated about those things. This can lead to analysis paralysis. You shouldn't be concerned though, since you can easily determine what a popular package is for what you're trying to solve, and try it out yourself. I think it's a positive since you're not limited to what the framework offers you.
...
In reality, React is just a humble library at ~2.8kb. It consists of two parts. The first being the [react](https://www.npmjs.com/package/react) package which is a diffing library (fancy term for some algorithm that is responsible to keep track of what changed, and perform an update based on that information). The other part is the [react-dom](https://www.npmjs.com/package/react-dom) package. A chonker at ~39.4kb responsible for everything DOM (document object model) related, and is meant to be paired with the `react` package.

Interesting thing to note is that because React is just a diffing algorithm, we can use it on any device. That's how [React Native](https://reactnative.dev/) allows us to create native apps for Android, and iOS using React. Instead of `react-dom`, where the DOM doesn't make sense on a phone, it uses native components.

## Why Should You Learn a Framework

If React is your first framework, this section is going to be enlightening.

If you have some experience with writing JavaScript, then you know how repetitive, tedious and error-prone things such as DOM manipulation can be. You're doing the same thing over and over. Everything is great on a fresh project, but soon enough things get out of hand. Your code becomes spaghetti. You might reach for a design pattern like OOP (object-oriented programming) to manage things easier. Even so performance is hard, and soon enough you're going to write your own hacked together framework before you know that you're now responsible of maintaining.

The goal of a framework is so you never have to touch the DOM, unless you have to. It makes writing code more declarative, meaning things are abstracted so you don't worry about implementation details. You can just focus on writing the logic. A good example is the [filter](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/filter) method in JavaScript. It's declarative since you don't think how it's implemented. You're just able to use it. The opposite would be imperative, where you'd write your own logic that implements the same.

Let's look at an example. We're going to display Pokemon with JavaScript, and compare it to the React equivalent. Not until we start, do we realize how something simple as that could not be so simple.

We have to consider a lot of things.

- Creation of DOM elements
- Populating DOM elements with content (not including setting attributes on them, such as `class` for styles)
- Outputing the content to the browser (meaning we have to target an element to do so)
- We have to attach, and keep track of an event listener to remove the Pokemon

Let's pretend we fetched the Pokemon data from some API, database, or JSON file. I encourage you to type this code out yourself so you feel the burn. Try this code out in [Codepen](https://codepen.io/), or whichever environment you prefer.
...
This is a lot of work already! Sure, we can abstract things further, and provide a nicer API to our developers, but now we're responsible for maintaing in essence a framework. We have to think about the implementation details, before writing "actual" code.
...
Let's look at the same example, done in React. **Don't stress having to understand React, I'm being vague on purpose to focus on concepts**.
...
_Brazenly taken from [Vue docs](https://v3.vuejs.org/guide/reactivity.html#what-is-reactivity) which are awesome, and I hope React docs could learn from._

## Virtual DOM

One thing I touched upon briefly was how we flushed the entire DOM for our list. With React we don't have to. It just updates what changed. We talked earlier how that's the entire point of the library. How does that work? It's thanks to to the [virtual DOM](https://reactjs.org/docs/faq-internals.html). You can think of it as a representation of the DOM in- memory. Once a change is made, React diffs it, and performs the update where needed.

We can see an example of this in the browser by enabling "Toggle paint flashing". I'm using Firefox, but you can find the same feature in Chrome.
...
You might have noticed something that looks like HTML in our React code. In reality, it's [JSX (JavaScript XML)](https://reactjs.org/docs/introducing-jsx.html). This let's us use our existing knowledge of HTML, with a few caveats which aren't relevant here.
...
Most React apps live inside a single div. That's why they're called single page application. This means that JavaScript has complete control. It swaps out content on the fly. For example you can use an API to fetch some data, and display it. It's also responsible for routing. We're not going to go into those details.
