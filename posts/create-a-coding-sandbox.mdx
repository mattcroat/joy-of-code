...
description: Create a JavaScript coding sandbox with plain TypeScript
published: '2021-9-15'
category: 'typescript'
...
- Show the **transpiled** output of [Babel](https://babeljs.io/) to be able to see the **JavaScript** created under the hood when using [React](https://reactjs.org/), or **transpiling** newer to older versions of **JavaScript** (EcmaScript)
...
You can visit the [JavaScript Sandbox](https://javascriptsandbox.netlify.app/) above, and find the [source code](https://github.com/joysofcode/javascript-sandbox) on **GitHub**.

## Table of Contents

1. [What's an Iframe?](#whats-an-iframe)
2. [Project Setup](#project-setup)
3. [Hooking Up The DOM](#hooking-up-the-dom)
4. [Updating the User Interface](#updating-the-user-interface)
5. [Adding Event Listeners](#adding-event-listeners)
6. [Showing Output](#showing-output)
7. [Showing Errors](#showing-errors)
8. [Using the Monaco Editor](#using-the-monaco-editor)
...
These are just using a fancy `<textarea>` that does **syntax highlighting**, and **code completion** among other things. **Codepen** uses [CodeMirror](https://codemirror.net/). **CodeSandbox** on the other hand is a lot more involved, and it's basically an **integrated development environment** (IDE) inside your browser. It uses the **Monaco Editor** just like **VS Code** That's what I decided to use for this project.
...
You can get the [starter and finished project files](https://github.com/joysofcode/javascript-sandbox), so drop it into your project. The styles are simple, and not worth going over but you can look at it if you want.
...
<Image
  height="1080px"
  width="1920px"
  src="/images/create-a-coding-sandbox/diagram.webp"
  alt="Codepen"
/>
...
Using **state** is going to make it simple to update the user interface, reduce errors, and impossible states (here we're just using **global variables**).
...
We're always going to know what state the application is thanks to `state`. Once we set state `state = 'editing'` we can update the user interface with `updateUI`.
...
You could also use an [enum](https://www.typescriptlang.org/docs/handbook/enums.html), or if using **JavaScript** a regular object.
...
We don't want to instantly update the **user interface** because it would be annoying for the user to instantly get an error on the screen as they type. Instead we're going to wrap `updateUI` with a `debounce` function that's going to wait at least **500ms**, or **0.5 seconds** before updating the **user interface**.

```js:src/utils/helpers.ts
...
The `debounce` function accepts a `callback` function it executes inside `setTimeout`, and a `delay` with a default value of **500ms**. The reason it returns a function is because of [closures](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Closures), which creates a new environment each time when used to prevent unexpected behavior.
...
There's also a **loading indicator** inside `index.html` that shows, and gets removed after the page loads. We already have a reference to `loading` inside `src/utils/dom.ts`.
...
6. `hasImports` checks if there are **imports**, and `imports` find the **imports** that returns an **array** so we use `join` to return a **string** and separate each **imports** by a new line, or set it to an **empty string**
...
8. For `sourceCode` we replace `/*#__PURE__*/` **string** everywhere to reduce noise in the output since

The `format.ts` utils is nothing special — just some **regex** to match **imports** and the `/\/\*#__PURE__\*\//g` **string** we wanted to remove, and a `replace` function I created to save on typing.
...
Don't forget we have to invoke `updateUI` to render the **user interface**.
...
<Image
  height="748px"
  width="1143px"
  src="/images/create-a-coding-sandbox/iframe.webp"
  alt="Iframe"
/>
...
<Image
  height="748px"
  width="1143px"
  src="/images/create-a-coding-sandbox/source.webp"
  alt="Source"
/>
...
We already set up the **event listener** earlier to listen for **errors**, so the only thing left to do is to update `updateUI`.
...
<Image
  height="748px"
  width="1143px"
  src="/images/create-a-coding-sandbox/error.webp"
  alt="Error"
/>
...
**Service workers** let you do tasks in the background (it's like using multiple threads), so you can use the page. **Monaco Editor** uses **service workers** for all features of the **editor**.

Inside `lib` we're going to add `monacoEditor.ts`.

```ts:src/lib/monaco/monacoEditor.ts
...
If you don't care about this, and make the code cleaner you can just say `import * as monaco from 'monaco-editor'`. The `import * as monaco` part is because there's no **default export**, the **Monaco Editor** just imports a bunch of things we can use.

We're using **React** code as the default example to show **Babel**, and **ES modules** working. You could try any other **JavaScript** framework such as **Vue** if it can work with **ES modules**. I even managed to make **Svelte** work.
...
Lastly we create, and **export** it as `monacoEditor`. Now we just need to **import**, and use it inside `main.ts`.
...
- **Persist code** by using [local storage](https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage) (you would just need to save the input to **local storage**, and on **page load** check if it exists, then set the contents of the editor to use it)
- Use a framework
