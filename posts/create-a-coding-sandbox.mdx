...
**Update:** The [monaco-editor](https://www.skypack.dev/view/monaco-editor) package we're using from [Skypack](https://www.skypack.dev/) doesn't work at the moment. If you follow along you're going to have a working coding sandbox using the regular textarea. That's why those examples work, but not the final one using the Monaco editor. I plan to rework the post to resolve the issues as it's starting to become a theme.
