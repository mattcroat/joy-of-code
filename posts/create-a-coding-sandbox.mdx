...
<Iframe src="https://javascriptsandbox.netlify.app/" />

Visit [JavaScript Sandbox](https://javascriptsandbox.netlify.app/)
...
![Codepen](/images/create-a-coding-sandbox/codepen.png)
...
![Hello, World!](/images/create-a-coding-sandbox/hello-world.png)
...
![Transpiled code](/images/create-a-coding-sandbox/transpiled.png)
...
You don't need to understand regex. I know enough myself to be able to read it, since I rarely use it. When I need it, I just search for what I'm looking for, and it works out. First let's add some regex patterns at the top of `app.js`, right below where we grab the DOM elements.
...
```js:example.js
console.error = () => {
  // hijack log to output to DOM element
}
```
...
It's just a regular listener that we can place on the _window object_, and listen for error messages. It's almost that easy! In our case we have to attach the listener inside our iframe, and somehow send them up to the parent.

I had weird issues just using it on the parent itself where it would ignore certain types of errors. There's a handy [window.postMessage](https://developer.mozilla.org/en-US/docs/Web/API/Window/postMessage) method for doing such.
...
![Error message](/images/create-a-coding-sandbox/error.png)
...
![Monaco Editor](/images/create-a-coding-sandbox/monaco.png)
