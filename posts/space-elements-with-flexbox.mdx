...
description: How using Flexbox to space elements is easier than using margin
published: '2021-8-31'
category: 'css'
image: '/images/space-elements-with-flexbox/og-image.webp'
---

# Space Elements Using Flexbox Instead of Margin

In your **HTML** markup you often have repeating blocks of code such as **lists**, **cards**, or **posts**.
...
const moviesHtml = `
  <section>
    <h1>Samurai Movies</h1>
    <ul class="movies">
      ${movies.map(movie => `<li class="movie">${movie}</li>`)}
    </ul>
  </section>
`
```

We could set the `margin` for each `.movie` class but then it would also push the top element.
