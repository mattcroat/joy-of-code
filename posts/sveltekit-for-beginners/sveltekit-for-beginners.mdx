...
description: Learn full stack web development using SvelteKit and what problems web frameworks like SvelteKit solve compared to just using the base library they're based on
...
Right now we're in the era of meta-frameworks and tackling other problems around frontend such as tooling instead of reinventing how we build user interfaces where React's mental model of components already won years ago.
...
- The large JavaScript bundle could be the single point of faillure when loading the page
...
Because SvelteKit is built on web fundamentals using the [Request](https://developer.mozilla.org/en-US/docs/Web/API/Request) and [Response](https://developer.mozilla.org/en-US/docs/Web/API/Response) object of the [Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) it's not bound to any environment but uses [adapters](https://kit.svelte.dev/docs/adapters) that generate the output required for the target platform without configuration.
...
This just means that you can spend more time on MDN learning about web fundamentals and improve as a web developer instead of just learning a framework abstraction.
...
<blockquote>
  🐿️ You might be wondering when to use Svelte or Sveltekit and the answer is
  always SvelteKit even if you don't use most of the features you always have
  the option.
</blockquote>

There's a lot more to learn about SvelteKit and in the next section I'm going to show you how to build a full stack app using SvelteKit and [Prisma](https://www.prisma.io/) to talk to a SQL database.

We're going to use [SQLite](https://www.sqlite.org/index.html) that's just a regular file on your system requiring no setup but you can later switch to any local or hosted SQL database you want. 🪄
...
<blockquote>
  ...
  shouldn't be breaking changes but always practice reading the docs.
</blockquote>
...
Here it's creating the `Tweet`, `User` and `Liked` tables.

The tweets are linked to the `User` table by using `@relation` where the `userId` field matches the `id` field of `Tweet`.

In the same way the `Liked` table references the `User` so we can know what tweets they liked.

🖌️ At this point we can create the database from our schema.
...
🚀  Your database is now in sync with your schema. Done in 113ms
```
...
🖌️ The last step to do is install the [Prisma Client](https://www.prisma.io/docs/concepts/components/prisma-client) which is required to use Prisma and run `npx prisma db push` to generate the client and types.
...
🖌️ We need to add a seed script inside `package.json` and because there's growing pains in Node.js with the transition to ES modules we need to specify a loader.

I figured this out by reading the error and doing some good old fashioned detective work but I hope they expand the [Seeding your database](https://www.prisma.io/docs/guides/database/seed-database) until this gets resolved. 🕵️
...
🖌️ Generate the client that's tailored to our database.
...
🚀  Your database is now in sync with your schema. Done in 76ms

✔ Generated Prisma Client (3.10.0 | library)
```

<blockquote>
  🐿️ If `npx prisma db push` fails generating try installing the Prisma CLI with
  `npm i -D prisma` and run it again.
</blockquote>
...
<blockquote>
  🐿️ You can find the generated client and types if you dig inside
  `node_modules` and `@prisma+client/.prisma/client`.
</blockquote>
...
I added some nice transitions you can see later when we implement adding, removing and liking a tweet.
